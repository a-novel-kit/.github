...
product but is never part of it: the `a-novel` CLI, the reusable CI workflows, the organization's
meta-repositories.

The distinction that matters is product versus tooling. A library is shipped: its code reaches production
...
A **library** factors logic that several repositories share into one shared, versioned, owned place,
depended on by an exact version rather than copied. The bar to add to one is high. A shared dependency is
...
own reference; here it is simply the tool that operates the rest.
